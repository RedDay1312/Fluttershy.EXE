...
import { CONNECTOR_TOKEN_HEADER, ConnectorType, type CallToolOptions, type CallToolResult, type ToolArgs } from "./types.ts";
...
function env(key: string): string | undefined { const v = process.env[key]?.trim(); return v || undefined; }
function isLoopbackHost(host: string): boolean { return host === "localhost" || host === "127.0.0.1" || host === "[::1]"; }

type InboundContext = { token: string | null; publicHost: string | null; connectorsBase: string | null };
...
function tryGetRequest(): Request | null { try { return getRequest() ?? null; } catch { return null; } }
...
async function gatePost(ctx: InboundContext, body: Record<string, unknown>, token: string): Promise<{ status: number; json: GateJson }> {
  ...
  if (text) { try { json = JSON.parse(text) as GateJson; } catch { json = { ok: false, errorMessage: `gate non-JSON response (HTTP ${res.status}): ${text.slice(0, 200)}` }; } }
...
function missingAuthResult(ctx: InboundContext): CallToolResult {
  ...
  return { ok: false, data: null, loginRequired: true, errorMessage: "missing_connector_token: open this app through the edge gate " + "(the server must receive x-connector-access-token on the inbound request)", ...(loginUrl ? { loginUrl } : {}) };
}
