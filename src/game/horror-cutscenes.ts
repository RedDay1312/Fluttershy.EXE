...
function runCutscene(level: number) {
  ...
  // Reuse the existing freeze state so Phaser stops updating the player while
  // the cinematic is running. The custom layer is rendered above it.
  ...
  Object.assign(root.style, {
    ...
    background: "#020304", color: "#ddd", opacity: "0",
    transition: "opacity 280ms ease", pointerEvents: "auto",
  ...
  Object.assign(film.style, {
    ...
    background: "radial-gradient(circle at 50% 48%, rgba(35,42,39,.2), rgba(0,0,0,.96) 76%)",
  });

  const grain = el("div");
  Object.assign(grain.style, {
    position: "absolute", inset: "0", opacity: ".22", mixBlendMode: "screen",
    background: "repeating-linear-gradient(0deg, transparent 0 3px, rgba(255,255,255,.035) 4px, transparent 5px)",
  });

  const topBar = el("div"), bottomBar = el("div");
  ...
  Object.assign(title.style, {
    ...
    textTransform: "uppercase", opacity: "0", transform: "translateX(-24px)", transition: "opacity 600ms ease, transform 700ms ease",
  ...
  Object.assign(caption.style, {
    ...
    color: "rgba(220,225,222,.72)", fontFamily: "Georgia, serif", fontSize: "clamp(15px, 2vw, 23px)",
    lineHeight: "1.65", opacity: "0", transform: "translateY(18px)", transition: "opacity 700ms ease 500ms, transform 700ms ease 500ms",
  ...
  Object.assign(pony.style, {
    ...
    transform: "translate(-50%,-50%) scale(.82)", backgroundImage: "url('/sprites/fs-horror.png')",
    backgroundPosition: "center", backgroundRepeat: "no-repeat", backgroundSize: "contain",
    filter: "contrast(1.45) brightness(.34) saturate(.18) drop-shadow(0 0 40px rgba(0,0,0,.98))",
    opacity: "0", transition: "opacity 420ms ease, transform 1600ms cubic-bezier(.18,.8,.15,1)",
  });

  const progress = el("div");
  Object.assign(progress.style, {
    position: "absolute", right: "8vw", bottom: "8vh", zIndex: "5", color: "rgba(255,255,255,.28)",
    fontSize: "10px", letterSpacing: ".25em", fontFamily: "monospace",
  });
  ...
  if (level >= 3) playHorrorSfx("breath");
  if (level >= 5) window.setTimeout(() => playHorrorSfx("steps"), 900);
  if (level >= 6) window.setTimeout(() => playSfx("stinger"), 2400);

  // One-frame visual rupture near the end, rather than a predictable full-screen screamer.
  if (scene.image && level >= 3) {
    window.setTimeout(() => {
      ...
      playHorrorSfx("snap");
      window.setTimeout(() => {
        pony.style.opacity = "0";
        film.style.background = "#000";
      }, 95);
  ...
  // Escape is intentionally ignored: the player cannot skip the first few seconds.
...
bridge.on((e) => {
  if (e.type === "level-clear") {
    window.setTimeout(() => runCutscene(e.level), 520);
  }
});
