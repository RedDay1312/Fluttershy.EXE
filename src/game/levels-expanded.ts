...
function equestriaDetails(level: LevelDef, offset: number, pass: number): Decor[] {
  ...
  // Ponyville/Fluttershy's side of Equestria: familiar shapes first, horror second.
  if (level.id === 1) {
    ...
    // A recognizable Ponyville rhythm: little cottages + flowerbeds + wandering butterflies.
    ...
    // Familiar Equestrian props survive the corruption as unsettling anchors.
    ...
    // The later worlds should look like Equestria being overwritten, not a generic void.
  ...
  // Show-like visual cues: clusters of friendship symbols are deliberately repeated less often than rocks/eyes.
...
function atmosphericDecor(level: LevelDef, offset: number, pass: number): Decor[] {
  ...
  if (level.id >= 3) {
    ...
    for (let i = 0; i < count; i++) { const x = 560 + ((i * 977 + level.id * 313 + pass * 181) % Math.max(600, w - 900)); const y = 170 + ((i * 67 + pass * 29) % 120); const sprite = hanging[(i + level.id + pass) % hanging.length]; const alpha = level.id === 3 ? 0.48 : level.id === 4 ? 0.62 : Math.min(0.78, 0.48 + pass * 0.08); add(x, y, sprite, level.id >= 5 ? 0.72 : 0.62, 4, alpha, { sway: true }); }
  }
