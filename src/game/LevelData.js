...
export function buildLayout(index){const c=getChapter(index),platforms=[],segment=c.length/10;for(let i=0;i<10;i++){const x=Math.floor(segment*i+320);platforms.push({x,y:470-(i%3)*70,w:Phaser.Math.Between(280,560),h:28});if(i>0)platforms.push({x:x+Phaser.Math.Between(110,260),y:300-(i%2)*45,w:190,h:24});}const memories=Array.from({length:c.goal},(_,i)=>({x:700+i*((c.length-1300)/Math.max(1,c.goal-1)),y:220+(i%2)*105}));const traps=Array.from({length:6+index*2},(_,i)=>({x:900+i*((c.length-1200)/Math.max(1,5+index)),y:555,w:54,h:24}));return{platforms,memories,traps,exitX:c.length-260};}
