...
function noiseBurst(dest: GainNode, dur: number, gain = 0.05, at = 0) {
  ...
  const g = bus.ctx.createGain();
  const f = bus.ctx.createBiquadFilter();
  src.buffer = buffer; f.type = "bandpass"; f.frequency.value = 900;
...
export function setMusicBed(level: number) {
  ...
  master.connect(dest); musicNodes.push(master);
  const pad = (freq: number, type: OscillatorType, gain: number) => {
    ...
    osc.type = type; osc.frequency.value = freq; f.type = "lowpass"; f.frequency.value = level >= 4 ? 600 : 900; g.gain.value = gain;
    const lfo = ctx.createOscillator(); const lg = ctx.createGain(); lfo.frequency.value = 0.07 + level * 0.02; lg.gain.value = freq * 0.01;
    lfo.connect(lg); lg.connect(osc.frequency); osc.connect(f); f.connect(g); g.connect(master); osc.start(); lfo.start(); musicNodes.push(osc, g, f, lfo, lg);
  };
...
export function hushMusic(seconds = 0.4) {
  ...
  window.setTimeout(() => { if (bus && musicOn) bus.music.gain.setTargetAtTime(0.22, ctxNow(), 0.4); }, seconds * 1000);
}
