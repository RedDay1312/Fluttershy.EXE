...
export class PlayScene extends Phaser.Scene {
  ...
  lastDeathTalk = -1;
  ambientLookAt = 0;
  angel?: Phaser.Physics.Arcade.Sprite;
  angelGone = false;
  ...
  init(data: { level?: number }) {
    ...
    this.angelGone = useGameStore.getState().angelGone;
  ...
  create() {
    ...
    L.pickups.forEach((c) => {
      if (useGameStore.getState().notes.includes(c.id) && (c.kind === "note" || c.kind === "letter" || c.kind === "gem"))
        return;
      const key =
        c.kind === "butterfly"
          ? "butterflies"
      ...
      else if (c.kind === "gem") spr.setDisplaySize(40, 40);
      else if (c.kind === "mark") spr.setDisplaySize(40, 40);
    ...
    L.decor.forEach((d) => {
      ...
      if (d.sprite === "hang-yellow" || d.sprite.startsWith("hang-")) img.setOrigin(0.5, 0);
      if (d.sprite === "discord") img.setOrigin(0.5, 1);
      if (d.sway) {
        this.tweens.add({
          targets: img,
          angle: { from: -5, to: 5 },
    ...
    L.checkpoints.forEach((c) => {
      this.add.image(c.x, c.y + 80, "flag").setOrigin(0.5, 1).setDepth(5).setDisplaySize(28, 44);
    });
    ...
    const fg = this.add
      .tileSprite(0, L.height - 36, L.width, 64, "fg-grass")
      .setOrigin(0, 1)
      .setScrollFactor(1.08)
      .setDepth(28)
      .setAlpha(L.id === 1 ? 0.85 : L.id === 2 ? 0.55 : 0.25);
    void fg;
    ...
    if (L.angel && !this.angelGone) {
      this.angel = this.physics.add.sprite(this.spawn.x - 70, this.spawn.y, "angel", 0);
      this.angel.setDepth(19);
      this.angel.setDisplaySize(56, 56);
      this.angel.play("angel-anim");
      const body = this.angel.body as Phaser.Physics.Arcade.Body;
    ...
    this.physics.add.overlap(this.pony.sprite, this.pickups, (_p, item) => {
      ...
      if (kind === "note" || kind === "letter" || kind === "gem") bridge.emit({ type: "note", id });
        ...
        bridge.emit({ type: "collect", kind: "mark" });
        bridge.emit({ type: "toast", key: "toast.mark" });
      } else if (kind === "butterfly") {
        bridge.emit({ type: "collect", kind: "butterfly" });
        if (this.level.id >= 3 && Math.random() < 0.45) {
          playSfx("whisper");
          bridge.emit({ type: "whisper", key: "whisper.3" });
        }
    ...
    this.motes = this.add.particles(0, 0, "px", {
      ...
      y: { min: 0, max: 720 },
      lifespan: 5000,
      speedY: { min: L.id >= 3 ? 20 : -18, max: L.id >= 3 ? 70 : 10 },
  ...
  private kill() {
    ...
    this.time.delayedCall(520, () => {
      ...
      if (this.level.id >= 6) this.pony.setDistorted(true);
  ...
  private clearLevel() {
    ...
    this.time.delayedCall(720, () => {
      bridge.emit({ type: "interlude", after: this.level.id });
    });
  }
  ...
  private fireTrigger(t: LevelDef["triggers"][number], idx: number) {
    ...
    switch (t.event) {
      case "dialogue":
        if (t.key) bridge.emit({ type: "dialogue", key: t.key });
        break;
      case "look":
        this.pony.look(2400);
        hushMusic(2);
        playSfx("whisper");
        if (t.key) bridge.emit({ type: "dialogue", key: t.key, look: true });
        bridge.emit({ type: "overlay", kind: "look", ms: 1800 });
        break;
      case "stare":
        this.pony.look(3200);
        hushMusic(3);
        ...
        if (t.key) bridge.emit({ type: "dialogue", key: t.key, look: true });
        bridge.emit({ type: "overlay", kind: "stare", ms: 2400 });
        break;
      case "red":
        playSfx("stinger");
        this.cameras.main.shake(200, 0.01);
        bridge.emit({ type: "overlay", kind: "red", textKey: t.key, ms: 1600 });
        break;
      case "bsod":
        this.pausedLogic = true;
        this.physics.pause();
        playSfx("stinger");
        bridge.emit({ type: "overlay", kind: "bsod", textKey: "bsod.body", ms: 4200 });
        this.time.delayedCall(4200, () => {
          this.physics.resume();
          this.pausedLogic = false;
          bridge.emit({ type: "dialogue", key: "d.l4.after" });
        });
        break;
      case "freeze":
        this.pausedLogic = true;
        this.physics.pause();
        playSfx("stinger");
        bridge.emit({ type: "overlay", kind: "freeze", textKey: "freeze.body", ms: 7000 });
      ...
      case "black":
        ...
        this.physics.pause();
        hushMusic(3);
        bridge.emit({ type: "overlay", kind: "black", textKey: t.key ?? "black.1", ms: 3800 });
      ...
      case "glitch":
        ...
        bridge.emit({ type: "overlay", kind: "glitch", ms: 1400 });
        bridge.emit({ type: "shake-window" });
        break;
      case "whisper":
        playSfx("whisper");
        if (t.key) bridge.emit({ type: "whisper", key: t.key });
        break;
      case "shake":
        bridge.emit({ type: "shake-window" });
        ...
        break;
      case "notepad":
        bridge.emit({ type: "overlay", kind: "notepad", textKey: t.key ?? "notepad.1", ms: 3200 });
        break;
      case "windows":
        bridge.emit({ type: "overlay", kind: "windows", ms: 4000 });
        break;
      case "cursor":
  ...
  private vanishAngel() {
    if (this.angelGone || !this.angel) return;
    this.angelGone = true;
    playSfx("whisper");
    this.tweens.add({
      targets: this.angel,
      ...
      x: this.angel.x + 80,
      duration: 900,
      onComplete: () => this.angel?.destroy(),
    });
    bridge.emit({ type: "angel-gone" });
    bridge.emit({ type: "dialogue", key: "d.l2.angelgone" });
  }

  private updateNpcs(interact: boolean) {
    const px = this.pony.sprite.x;
    const py = this.pony.sprite.y;
    ...
    if (this.npcHint) {
      if (near) {
        ...
        this.npcHint.setPosition(near.x, near.y + 160);
      } else this.npcHint.setVisible(false);
    }
    if (interact && near) {
      const key = near.nameKey;
      if (!this.spokenNpc.has(key)) {
        this.spokenNpc.add(key);
        playSfx("whisper");
        bridge.emit({
          type: "dialogue",
          key: near.lineKey,
          speaker: "npc",
          nameKey: near.nameKey,
        });
      }
    }
  }
  ...
  update(_time: number, delta: number) {
    ...
    if (
      store.phase === "paused" ||
      store.overlay.kind === "bsod" ||
      store.overlay.kind === "freeze" ||
      store.overlay.kind === "black" ||
      store.overlay.kind === "stare"
    ) {
      this.pony.lock(true);
      return;
    }
    ...
    this.level.checkpoints.forEach((c) => {
      if (Math.abs(this.pony.sprite.x - c.x) < 48 && this.pony.grounded()) {
        if (this.spawn.x !== c.x) {
          this.spawn = { x: c.x, y: c.y };
          bridge.emit({ type: "checkpoint", level: this.level.id, x: c.x, y: c.y });
        }
      }
    });
    ...
    if (this.angel && !this.angelGone) {
      const tx = this.pony.sprite.x - this.pony.facing * 64;
      const ty = this.pony.sprite.y + 18;
      this.angel.x += (tx - this.angel.x) * Math.min(1, dt * 3.2);
      this.angel.y += (ty - this.angel.y) * Math.min(1, dt * 4);
      this.angel.setFlipX(this.pony.facing < 0);
    }

    this.flicker.forEach((p, i) => {
      const on = Math.sin(_time / 180 + i) > -0.65;
      p.setAlpha(on ? 1 : 0.15);
      (p.body as Phaser.Physics.Arcade.StaticBody).enable = on;
    });
    ...
    this.movers.forEach((m) => {
      ...
      const vx = (nx - m.spr.x) / Math.max(dt, 0.001);
      const vy = (ny - m.spr.y) / Math.max(dt, 0.001);
    ...
    this.watchers.forEach((w) => {
      w.setFlipX(px < w.x);
      if (w.texture.key === "eyes") w.setAlpha(0.55 + Math.sin(_time / 500 + w.x) * 0.35);
    });
    ...
    if (this.level.id >= 4 && Math.random() < 0.004) {
      this.cameras.main.shake(80, 0.002);
    }

    const cam = this.cameras.main;
    cam.setFollowOffset(this.pony.facing > 0 ? -90 : 90, 36);
  }
}
